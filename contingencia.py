...
def parsear_fecha_operacion(val):
    """Parsea fechas DD/MM/YYYY del Excel con máxima fidelidad."""
    if pd.isna(val):
        return pd.NaT
...
def procesar_auditoria_completa(df_ops, df_cal, digito_usuario, fecha_contingencia, tipo_cierre_usuario):
    ...
    if col_doc:
        nombre_col_fecha_temp = [c for c in df_ops.columns if 'fecha' in c.lower() and 'operaci' in c.lower()]
        nombre_col_doc = col_doc[0]
        total_previo = len(df_ops)
        df_ops['doc_temp'] = df_ops[nombre_col_doc].astype(str).str.strip().replace(
            {'nan': pd.NA, '': pd.NA, 'None': pd.NA, '0': pd.NA}
        )
        df_con_doc = df_ops.dropna(subset=['doc_temp'])
        df_sin_doc = df_ops[df_ops['doc_temp'].isna()]

        # ── FIX: ordenar por fecha real antes de deduplicar ──
        if nombre_col_fecha_temp:
            df_con_doc = df_con_doc.copy()
            df_con_doc['_fecha_sort'] = df_con_doc[nombre_col_fecha_temp[0]].apply(parsear_fecha_operacion)
            df_con_doc = df_con_doc.sort_values('_fecha_sort', ascending=True)
            df_con_doc = df_con_doc.drop_duplicates(subset=['doc_temp', 'Renta'], keep='last')
            df_con_doc = df_con_doc.drop(columns=['_fecha_sort'])
        else:
            df_con_doc = df_con_doc.drop_duplicates(subset=['doc_temp', 'Renta'], keep='last')
