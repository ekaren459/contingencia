...
def parsear_fecha_operacion(val):
    """
    Parsea fechas del Excel cubriendo todos los casos posibles:
    - np.datetime64 (lo que pandas genera al leer Excel con columnas de fecha)
    - datetime / Timestamp nativos
    - Número serial de Excel (float como 45678.0)
    - Strings: DD/MM/YYYY, YYYY-MM-DD HH:MM:SS, DD-MM-YYYY, etc.
    """
    # Nulos
    try:
        if pd.isna(val):
    ...
    # np.datetime64 — caso más común al leer Excel con pandas
    ...
    # datetime / Timestamp ya parseado
    if isinstance(val, (datetime, pd.Timestamp)):
        return pd.Timestamp(val)

    # Número serial de Excel (cuando la celda viene como float)
    ...
    # String — limpiar primero
    val_str = str(val).strip()
    if not val_str or val_str.lower() in ('nan', 'none', 'nat', 'nd', '-', ''):
        return pd.NaT

    # Quitar hora si viene como "15/03/2024 00:00:00" o "2024-03-15 00:00:00"
    val_str = val_str.split(' ')[0].strip()

    # Intentar formatos explícitos en orden de probabilidad
    for fmt in (
        '%d/%m/%Y',   # 15/03/2024  <- más común en Venezuela
        '%d-%m-%Y',   # 15-03-2024
        '%Y-%m-%d',   # 2024-03-15  <- openpyxl a veces genera esto
        '%d/%m/%y',   # 15/03/24
        '%m/%d/%Y',   # 03/15/2024
        '%Y/%m/%d',   # 2024/03/15
    ):
    ...
    # Último recurso: inferencia con dayfirst=True
    return pd.to_datetime(val_str, errors='coerce', dayfirst=True)


def parse_periodo(val):
    """Parsea el campo Periodo en distintos formatos incluyendo DD/MM/YYYY y MM/YYYY."""
    if pd.isna(val):
        return 0, 0
    # np.datetime64 — caso más común al leer Excel
    if isinstance(val, np.datetime64):
        try:
            ts = pd.Timestamp(val)
...
def procesar_auditoria_completa(df_ops, df_cal, digito_usuario, fecha_contingencia, tipo_cierre_usuario):
    ...
    # No se eliminan duplicados por número de documento.
    # Cada fila del Excel se toma como un registro independiente.
    ...
    # ── FIX: usar parsear_fecha_operacion para máxima fidelidad DD/MM/YYYY ──
    ...
    def es_registro_valido(row):
        ...
        # IVA/30: excluir filas con "crédito" en el concepto
        if row['Renta_Norm'] == 'IVA/30':
            if 'credito' in texto_completo or 'crédito' in texto_completo:
                return False
            return True

        # ISLR (anual o retención): excluir filas con "sustitutiva" en descripción
    ...
    if not df_ops.empty:
        ...
        # ── FIX: ordenar por fecha real ANTES del cumcount ──
        ...
        def deducir_quincena(row):
            ...
            if row['Renta_Norm'] == 'ANTICIPO-ISLR':
                ...
                # Usar quincena explícita si viene en el Excel
                if row['Quincena_Original'] in ['Primera', 'Segunda']:
                    return row['Quincena_Original']
                # Usar fecha_limite del calendario: día ≤ 20 → Primera, día > 20 → Segunda
                if pd.notna(row.get('fecha_limite')):
                    return 'Primera' if row['fecha_limite'].day <= 20 else 'Segunda'
                # Fallback: orden cronológico por fecha real de pago
                return 'Primera' if row['Orden_Declaracion'] == 1 else 'Segunda'
...
if True:  # siempre mostrar la card hasta que haya archivo
    st.markdown("""
