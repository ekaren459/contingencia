...
[data-testid="stFileUploaderDropzone"] {
    background: #ffffff !important; border: 2px dashed #c7d2e8 !important;
...
[data-testid="stFileUploaderDropzone"]:hover {
    border-color: #3b82f6 !important; background: #f8faff !important;
    box-shadow: 0 8px 40px rgba(37,99,235,0.12), 0 2px 8px rgba(0,0,0,0.06) !important;
    transform: translateY(-2px) !important;
}
[data-testid="stFileUploaderDropzoneInstructions"] { font-size: 0.9rem !important; color: #64748b !important; }
...
.expediente-header {
    ...
    padding: 1.4rem 2rem; margin-bottom: 1rem; display: flex;
...
.trans-badge {
    ...
    font-size: 0.84rem; color: #1e3a5f; font-weight: 600;
...
.upload-hero-title {
    font-family: 'Fraunces', serif; font-size: 1.5rem; font-weight: 700; color: #ffffff;
    margin: 0 0 0.4rem; letter-spacing: -0.02em; position: relative; z-index: 1;
}
.upload-hero-sub { font-size: 0.85rem; color: rgba(255,255,255,0.7); margin: 0; position: relative; z-index: 1; }
.upload-hero-body { padding: 1.8rem 2.5rem 2rem; }
.upload-chips { display: flex; gap: 0.6rem; flex-wrap: wrap; margin-bottom: 1.4rem; }
.upload-chip {
    display: inline-flex; align-items: center; gap: 6px;
    background: #f1f5f9; border: 1px solid #e2e8f0; border-radius: 999px;
    padding: 5px 13px; font-size: 0.76rem; color: #475569; font-weight: 600;
}
...
def _obtener_ultima_tasa_db(moneda: str):
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                """SELECT tasa, fecha_bcv, registrado FROM tasa_bcv_historico
                   WHERE moneda = %s ORDER BY registrado DESC, id DESC LIMIT 1""",
                (moneda,)
            )
            row = cur.fetchone()
        conn.close()
...
def obtener_tasas_bcv():
    ...
    if eur.get("tasa") is None:
        st.warning("⚠️ No se pudo obtener la tasa **EUR** del BCV y tampoco hay datos guardados en la BD.")
...
def parse_periodo(val):
    ...
    if isinstance(val, np.datetime64):
        try: ts = pd.Timestamp(val); return ts.month, ts.year
        except Exception: return 0, 0
    if isinstance(val, (datetime, pd.Timestamp)):
        return val.month, val.year
    val_str = str(val).lower().strip()
    m = re.search(r'(\d{1,2})[/\-](\d{1,2})[/\-](20\d{2})', val_str)
    if m: return int(m.group(2)), int(m.group(3))
    m = re.search(r'(\d{1,2})[/\-](20\d{2})', val_str)
    if m: return int(m.group(1)), int(m.group(2))
    meses_map = {'ene':1,'feb':2,'mar':3,'abr':4,'may':5,'jun':6,
                 'jul':7,'ago':8,'sep':9,'oct':10,'nov':11,'dic':12}
    mes, anio = 0, 0
    m = re.search(r'(20\d{2})', val_str)
    if m: anio = int(m.group(1))
    else:
        m2 = re.search(r'(?<!\d)(\d{2})(?!\d)', val_str)
        if m2: anio = 2000 + int(m2.group(1))
    for ms, mn in meses_map.items():
        if ms in val_str: mes = mn; break
    if mes == 0:
        m = re.search(r'^(0?[1-9]|1[0-2])[-/]', val_str)
        if m: mes = int(m.group(1))
    return mes, anio


def _detectar_quincena_por_proximidad(fecha_pago, df_cal_crudo, digito, trans_mes, trans_anio):
    """
    Compara la fecha real del primer pago especial contra las fechas límite
    oficiales de Primera y Segunda quincena en el calendario del SENIAT.
    Devuelve la quincena cuya fecha límite esté más cercana al pago.
    """
    if fecha_pago is pd.NaT or fecha_pago is None:
        return "Primera"
    try:
        mask = (
            (df_cal_crudo['digito_rif'] == digito) &
            (df_cal_crudo['mes'] == trans_mes) &
            (df_cal_crudo['anio'] == trans_anio) &
            (df_cal_crudo['tipo_contribuyente'] == 'especial') &
            (df_cal_crudo['quincena'].isin(['Primera', 'Segunda']))
        )
        df_q = df_cal_crudo[mask][['quincena', 'fecha_limite']].dropna().copy()
        if df_q.empty:
            return "Segunda" if fecha_pago.day > 15 else "Primera"
        df_q['distancia'] = df_q['fecha_limite'].apply(lambda fl: abs((fecha_pago - fl).days))
        return df_q.loc[df_q['distancia'].idxmin(), 'quincena']
    except Exception:
        return "Primera"
...
def procesar_auditoria_completa(df_ops, df_cal, digito_usuario, fecha_contingencia, tipo_cierre_usuario):
    ...
    df_cal_crudo          = df_cal.copy()   # copia para consultas de proximidad
    ...
    df_ops['Es_Sustitutiva']   = df_ops.astype(str).apply(
        lambda col: col.str.lower().str.contains('sustitutiva')
    ).any(axis=1)
    ...
    def es_registro_valido(row):
        ...
        if row['Renta_Norm'] == 'IVA/30':
            if 'credito' in texto_completo or 'crédito' in texto_completo: return False
            return True
        if row['Renta_Norm'] == 'ISLR-ANUAL':
            return True   # sustitutivas gestionadas en determinar_estatus
        if row['Renta_Norm'] == 'ISLR-RETENCION':
            if 'sustitutiva' in texto_completo: return False
            return True
        return True
    ...
    df_ops = df_ops[
        (df_ops['Fecha_Operacion_Real'].isna()) |
        (df_ops['Fecha_Operacion_Real'] <= fecha_contingencia_dt)
    ].copy()

    anticipos_por_mes  = {}
    info_transicion    = {}   # ← guardará detalles del primer pago especial
    ...
    if not df_ops.empty:
        ...
        if not df_esp_iva35.empty:    candidatos_transicion.append(df_esp_iva35['YYYYMM'].min())
        if not df_esp_anticipo.empty: candidatos_transicion.append(df_esp_anticipo['YYYYMM'].min())
        ...
        if candidatos_transicion:
            ...
            if trans_yyyymm > min_general_yyyymm:
                trans_mes_num  = int(str(trans_yyyymm)[-2:])
                trans_anio_num = int(str(trans_yyyymm)[:4])
                mes_t          = f"{trans_mes_num:02d}"
                anio_t         = str(trans_anio_num)

                # ── Detectar quincena del primer pago especial por proximidad al calendario ──
                df_trans_pagos  = df_ops[df_ops['YYYYMM'] == trans_yyyymm].sort_values(
                    'Fecha_Operacion_Real', ascending=True, na_position='last'
                )
                fecha_pago_trans = pd.NaT
                fechas_validas   = df_trans_pagos['Fecha_Operacion_Real'].dropna()
                if not fechas_validas.empty:
                    fecha_pago_trans = fechas_validas.iloc[0]

                # Identificar qué impuesto disparó la transición
                renta_trans = df_trans_pagos.iloc[0]['Renta_Norm'] if not df_trans_pagos.empty else "—"

                quincena_trans = _detectar_quincena_por_proximidad(
                    fecha_pago_trans, df_cal_crudo, digito_usuario, trans_mes_num, trans_anio_num
                )

                tipo_final_empresa = f"Transición a Especial ({mes_t}/{anio_t} — inicia {quincena_trans} quincena)"

                # Guardar detalles para el badge de transición
                info_transicion = {
                    "periodo":    f"{mes_t}/{anio_t}",
                    "quincena":   quincena_trans,
                    "impuesto":   renta_trans,
                    "fecha_pago": fecha_pago_trans.strftime('%d/%m/%Y') if fecha_pago_trans is not pd.NaT else "—",
                    "meses_ord":  len(df_ops[df_ops['Tipo_Contribuyente'] == 'ordinario']['YYYYMM'].unique()),
                }
            else:
    ...
    df_ops_cruce = df_ops if not df_ops.empty else pd.DataFrame(
        columns=['Digito','Renta_Norm','Anio_Cruce','Mes_Cruce','Quincena_Deducida']
    )
    ...
    def determinar_estatus(row):
        ...
        if row['_merge'] == 'left_only':
            ...
            concepto_val = str(row.get(nombre_concepto, '')).lower()
            desc_col     = 'Descripción' if 'Descripción' in row.index else 'descripcion'
            desc_val     = str(row.get(desc_col, '')).lower()
            texto_fila   = concepto_val + ' ' + desc_val
            if row['Renta_Norm'] == 'ISLR-ANUAL' and 'sustitutiva' in texto_fila:
                return '🟡 Excluido (Sustitutiva — sin multa)'
    ...
    df_audit['Días de Desviación'] = np.where(
        df_audit['Estatus_Fiscal'].str.contains('Exonerado|Excluido'), 0,
        np.where(
    ...
    df_audit['Monto(Bs.)'] = pd.to_numeric(
        df_audit[col_monto[0]], errors='coerce'
    ).fillna(0.0).round(2) if col_monto else 0.0
    ...
    # ── Multas por pago tardío (Art. 103 / 108) ──
    mask_tarde    = df_audit['Estatus_Fiscal'].str.contains('Pagado Tarde') & ~df_audit['Estatus_Fiscal'].str.contains('Excluido', na=False)
    mask_especial = df_audit['Fase_Fiscal'].str.lower() == 'especial'
    df_audit['Multa_Art_103(EUR)'] = np.where(mask_tarde, 100.0, 0.0)
    df_audit['Multa_Art_108(EUR)'] = np.where(mask_tarde & mask_especial, 200.0, 0.0)

    # ── NUEVO: Multa por omisión de IVA (solo IVA/30 e IVA/35 NO PAGADOS Vencidos) ──
    # 100 EUR ordinario / 200 EUR especial por cada declaración omitida
    mask_iva_omitido = (
        df_audit['Estatus_Fiscal'].str.contains('NO PAGADO \\(Vencido\\)', na=False) &
        df_audit['Renta_Norm'].isin(['IVA/30', 'IVA/35'])
    )
    df_audit['Multa_Omision_IVA(EUR)'] = np.where(
        mask_iva_omitido & mask_especial, 200.0,
        np.where(mask_iva_omitido, 100.0, 0.0)
    )

    df_audit['Multas_Fijas(EUR)'] = (
        df_audit['Multa_Art_103(EUR)'] + df_audit['Multa_Art_108(EUR)'] + df_audit['Multa_Omision_IVA(EUR)']
    ).round(2)
    ...
    mask_excluido = df_audit['Estatus_Fiscal'].str.contains('Excluido', na=False)
    df_audit.loc[mask_excluido, 'Multa_Acumulada(Bs.)'] = 0.0
    df_audit.loc[mask_excluido, 'Total_Deuda(Bs.)']     = 0.0
    ...
    df_audit['Concepto Contable'] = (
        df_audit[nombre_concepto].fillna('— Sin Registro —')
        if nombre_concepto in df_audit.columns else '— Sin Registro —'
    )
    df_audit['Periodo Registrado en Excel'] = (
        df_audit['Periodo_Original'].fillna('— No Pagado —')
    ...
    df_audit['Fecha de Pago Real']   = (
        df_audit['Fecha_Operacion_Real'].dt.strftime('%d/%m/%Y').fillna('— No Pagado —')
    ...
    for col in ['Monto(Bs.)','Multa_Acumulada(Bs.)','Multa_Art_103(EUR)','Multa_Art_108(EUR)',
                'Multa_Omision_IVA(EUR)','Multas_Fijas(EUR)','Multas_Fijas(Bs.)',
    ...
    columnas_finales = [
        ...
        'Multa_Omision_IVA(EUR)','Multas_Fijas(Bs.)','Total_Deuda(Bs.)','Tasa_BCV_EUR','Deuda_Total(EUR)'
    ]
    ...
    info_expediente = {
        ...
        "info_transicion":       info_transicion,
    }
...
def generar_excel_multitabla(df_detalles):
    COLS_NUM = ['Monto(Bs.)','Multa_Acumulada(Bs.)','Multa_Art_103(EUR)','Multa_Art_108(EUR)',
                'Multa_Omision_IVA(EUR)','Multas_Fijas(Bs.)','Total_Deuda(Bs.)','Tasa_BCV_EUR','Deuda_Total(EUR)']
    ...
    def aplicar_formato_excel(ws):
        ...
        for row_idx, row in enumerate(ws.iter_rows(min_row=2), start=2):
            for cell in row:
                if row_idx % 2 == 0:
                    if not cell.fill or cell.fill.fill_type == 'none': cell.fill = light_fill
                cell.border = border
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        for fase in df_export['Fase_Fiscal'].dropna().unique():
            sn = f'Fase {fase}'
            df_export[df_export['Fase_Fiscal'] == fase].to_excel(writer, index=False, sheet_name=sn)
        ...
        if not df_cp.empty:
            ...
            cols_f = ['Fase_Fiscal','Periodo','Impuesto','Frecuencia','Fecha Límite Oficial',
                      'Fecha de Pago Real','Días de Mora/Desviación','Monto(Bs.)','Estatus_Fiscal']
            df_fe = df_cp[[c for c in cols_f if c in df_cp.columns]].copy()
            df_fe['Diagnóstico Puntualidad'] = df_fe['Días de Mora/Desviación'].apply(diag_export)
            df_fe.to_excel(writer, index=False, sheet_name='Auditoría de Fechas')
            aplicar_formato_excel(writer.sheets['Auditoría de Fechas'])
            fases_f = df_fe['Fase_Fiscal'].dropna().unique()
            if len(fases_f) > 1:
                for fase in fases_f:
                    sn2 = f'Fechas {fase}'
                    df_fe[df_fe['Fase_Fiscal'] == fase].to_excel(writer, index=False, sheet_name=sn2)
                    aplicar_formato_excel(writer.sheets[sn2])
...
if True:
    ...
    archivo_subido = st.file_uploader(
        ...
        type=["xlsx"], help="El archivo debe tener encabezados en la fila 4 (header=3)",
    ...
    if archivo_subido is None:
        st.markdown("""
        <div style="display:flex;gap:1rem;flex-wrap:wrap;margin-top:1.2rem;justify-content:center;">
            <div style="background:#ffffff;border:1px solid #e2e8f0;border-radius:14px;padding:1.2rem 1.6rem;
                        flex:1;min-width:180px;max-width:240px;box-shadow:0 1px 4px rgba(0,0,0,0.05);text-align:center;">
...
COLS_EUR = ['Multa_Art_103(EUR)','Multa_Art_108(EUR)','Multa_Omision_IVA(EUR)',
            'Multas_Fijas(EUR)','Deuda_Total(EUR)','Tasa_BCV_EUR']
...
def renderizar_tablero_fase(titulo_fase, emoji, df_reporte_fase, df_cal_fase, anios):
    ...
    c3.metric("A Tiempo / Exonerado", len(df_reporte_fase[df_reporte_fase['Estatus_Fiscal'].str.contains('Adelantado|Exacto|Exonerado|Excluido', na=False)]))
    ...
    def color_estatus(val):
        ...
        elif 'Excluido' in v:   return 'background-color:#fefce8;color:#713f12;font-weight:600'
        elif 'Por Vencer' in v: return 'background-color:#eff6ff;color:#1e3a5f;font-weight:600'
        return ''

    with pestanas[0]:
        st.dataframe(aplicar_formato_visual(df_reporte_fase).map(color_estatus, subset=['Estatus_Fiscal']), use_container_width=True, hide_index=True)
    for i, imp in enumerate(impuestos):
        with pestanas[i + 1]:
            st.dataframe(aplicar_formato_visual(df_reporte_fase[df_reporte_fase['Impuesto'] == imp]).map(color_estatus, subset=['Estatus_Fiscal']), use_container_width=True, hide_index=True)
    ...
    for anio in sorted(anios):
        ...
        if esperados > 0:
            ...
            with st.expander(f"{icono} {anio}  ·  Exigidas: {esperados}  ·  Cumplidas: {esperados - n_omitidos}  ·  Pendientes: {n_omitidos}"):
                if n_omitidos > 0:
                    st.dataframe(omitidos.groupby(['Impuesto','Frecuencia']).size().reset_index(name='Declaraciones Pendientes'), hide_index=True, use_container_width=True)
                else:
                    st.success(f"✅ Todas las {esperados} obligaciones cumplidas al corte.")


# ── DESGLOSE FINANCIERO CON CONTEO POR FILA ──────────────────
def construir_desglose(df_fuente, tasa, tasa_oficial):
    """
    Construye la tabla de impacto financiero.
    Columna extra 'N° Declaraciones' muestra cuántas declaraciones
    generan cada tipo de multa, con detalle IVA/30 + IVA/35 donde aplique.
    La fila TOTAL suma todas las declaraciones con algún tipo de multa o deuda.
    """
    # ── Montos base ──────────────────────────────────────────
    mask_nopag   = df_fuente['Estatus_Fiscal'].str.contains('NO PAGADO|Pendiente', na=False)
    mask_tarde   = df_fuente['Estatus_Fiscal'].str.contains('Pagado Tarde', na=False)
    mask_vencido = df_fuente['Estatus_Fiscal'].str.contains('NO PAGADO \\(Vencido\\)', na=False)

    monto_p = df_fuente[mask_nopag]['Monto(Bs.)'].sum()
    ...
    col_omision       = 'Multa_Omision_IVA(EUR)'
    omision_total_eur = df_fuente[col_omision].sum() if col_omision in df_fuente.columns else 0.0

    # ── Conteos para cada tipo de multa ──────────────────────

    # 1) Deuda principal: declaraciones con impuesto omitido (NO PAGADO vencido)
    n_deuda = int(mask_vencido.sum())

    # 2) Mora (5%): declaraciones con días de mora > 0
    n_mora = int((df_fuente.get('Días de Mora/Desviación', pd.Series(0, index=df_fuente.index)) > 0).sum()) \
             if 'Días de Mora/Desviación' in df_fuente.columns \
             else int((df_fuente['Multa_Acumulada(Bs.)'] > 0).sum())

    # 3) Art. 103 (100 EUR): declaraciones con esa multa > 0
    n_103  = int((df_fuente['Multa_Art_103(EUR)'] > 0).sum()) if 'Multa_Art_103(EUR)' in df_fuente.columns else 0
    # detalle por impuesto tardío
    if 'Impuesto' in df_fuente.columns and n_103 > 0:
        tard_por_imp = (
            df_fuente[df_fuente['Multa_Art_103(EUR)'] > 0]
            .groupby('Impuesto').size().to_dict()
        )
        detalle_103 = '  (' + ', '.join(f"{k} ×{v}" for k, v in tard_por_imp.items()) + ')'
    else:
        detalle_103 = ''

    # 4) Art. 108 (200 EUR): declaraciones especiales tardías
    n_108  = int((df_fuente['Multa_Art_108(EUR)'] > 0).sum()) if 'Multa_Art_108(EUR)' in df_fuente.columns else 0
    if 'Impuesto' in df_fuente.columns and n_108 > 0:
        tard108_imp = (
            df_fuente[df_fuente['Multa_Art_108(EUR)'] > 0]
            .groupby('Impuesto').size().to_dict()
        )
        detalle_108 = '  (' + ', '.join(f"{k} ×{v}" for k, v in tard108_imp.items()) + ')'
    else:
        detalle_108 = ''

    # 5) Omisión IVA: conteo desglosado IVA/30 + IVA/35
    mask_iva30 = df_fuente['Impuesto'].str.contains('IVA/30', na=False) if 'Impuesto' in df_fuente.columns else pd.Series(False, index=df_fuente.index)
    mask_iva35 = df_fuente['Impuesto'].str.contains('IVA/35', na=False) if 'Impuesto' in df_fuente.columns else pd.Series(False, index=df_fuente.index)
    n_omit_30  = int((mask_vencido & mask_iva30).sum())
    n_omit_35  = int((mask_vencido & mask_iva35).sum())
    n_omit     = n_omit_30 + n_omit_35
    detalle_omit = ''
    if n_omit_30 > 0: detalle_omit += f'IVA/30 ×{n_omit_30}'
    if n_omit_35 > 0: detalle_omit += (', ' if detalle_omit else '') + f'IVA/35 ×{n_omit_35}'
    if detalle_omit: detalle_omit = f'  ({detalle_omit})'

    # 6) Total de declaraciones únicas con algún impacto
    mask_con_impacto = (
        mask_nopag |
        mask_tarde |
        (df_fuente['Multa_Acumulada(Bs.)'] > 0) |
        (df_fuente.get('Multa_Omision_IVA(EUR)', 0) > 0)
    )
    n_total = int(mask_con_impacto.sum())

    # ── Filas de la tabla ─────────────────────────────────────
    def fmt_n(n, detalle=''):
        return f"{n} decl.{detalle}" if n > 0 else "—"
    ...
    rows = [
        ...
        (
            "📈 Contingencia por Materialidad (5% diario s/monto)",
            fmt_n(n_mora),
            f"Bs. {mora:,.2f}",
            f"€ {mora/tasa:,.2f}" if tasa_oficial else "—",
        ),
        (
            "📄 Incumplimiento Deber Formal — 100 EUR (pago tardío)",
            fmt_n(n_103, detalle_103),
            f"Bs. {a103*tasa:,.2f}",
            f"€ {a103:,.2f}",
        ),
        (
            "🏢 Incumplimiento Deber Formal — 200 EUR adicional (Especiales tardío)",
            fmt_n(n_108, detalle_108),
            f"Bs. {a108*tasa:,.2f}",
            f"€ {a108:,.2f}",
        ),
        (
            f"🧾 Omisión IVA (declaraciones no presentadas)",
            fmt_n(n_omit, detalle_omit),
            f"Bs. {omision_total_eur*tasa:,.2f}",
            f"€ {omision_total_eur:,.2f}",
        ),
        (
    ...
    return pd.DataFrame(rows, columns=["Concepto", "N° Declaraciones", "Monto (Bs.)", "Equivalente (EUR)"])
...
if archivo_subido is not None and not df_calendario.empty:
    ...
    # ── Badge de verificación de etapa de transición ──
    it = info.get("info_transicion", {})
    if it:
        icono_q  = "1️⃣" if it.get("quincena") == "Primera" else "2️⃣"
        imp_disp = it.get("impuesto", "—").replace("IVA/35", "IVA/35 (Especial)").replace("ANTICIPO-ISLR", "Anticipo ISLR")
        st.markdown(f"""
        <div class="trans-badge">
            🔄 <strong>Verificación de Transición</strong>
            &nbsp;·&nbsp; Periodo de inicio:
            <span class="pill">📅 {it.get('periodo','—')}</span>
            &nbsp;·&nbsp; Quincena detectada:
            <span class="pill-q">{icono_q} {it.get('quincena','—')}</span>
            &nbsp;·&nbsp; Impuesto que disparó cambio:
            <span class="pill" style="background:#7c3aed;">{imp_disp}</span>
            &nbsp;·&nbsp; Fecha del primer pago especial:
            <strong>{it.get('fecha_pago','—')}</strong>
            &nbsp;·&nbsp; Meses como Ordinario previos:
            <strong>{it.get('meses_ord', 0)}</strong>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        "y la fecha límite oficial. Solo registros con fecha de pago registrada."
        "</p>", unsafe_allow_html=True
    )
    ...
    if not df_con_pago.empty:
        ...
        def diagnostico_fecha(row):
            if 'Excluido' in str(row.get('Estatus_Fiscal','')): return '🟡 Excluido (Sustitutiva)'
        ...
        tiene_transicion          = len(df_fechas['Fase_Fiscal'].dropna().unique()) > 1

        def render_tabla_fechas(df_f, titulo):
            st.markdown(f"##### {titulo}")
            df_se = df_f[~df_f['Estatus_Fiscal'].str.contains('Excluido', na=False)]
            total     = len(df_f)
            tarde     = len(df_se[df_se['Días de Mora/Desviación'] > 0])
            exacto    = len(df_se[df_se['Días de Mora/Desviación'] == 0])
            adelant   = len(df_se[df_se['Días de Mora/Desviación'] < 0])
            excluidos = len(df_f[df_f['Estatus_Fiscal'].str.contains('Excluido', na=False)])
            pct_ok    = round((exacto + adelant) / max(total - excluidos, 1) * 100, 1)
            m1,m2,m3,m4,m5,m6 = st.columns(6)
            m1.metric("Total Pagos", total); m2.metric("🔴 Tardíos", tarde)
            m3.metric("🔵 Exactos", exacto); m4.metric("🟢 Anticipados", adelant)
            m5.metric("🟡 Excluidos", excluidos); m6.metric("✅ Cumplimiento", f"{pct_ok}%")
            cols_m = ['Periodo','Impuesto','Frecuencia','Fecha Límite Oficial',
                      'Fecha de Pago Real','Días de Mora/Desviación','Diagnóstico','Monto(Bs.)']
            def color_diag(v):
                v = str(v)
                ...
                return ''
            fmt_f = {'Días de Mora/Desviación':'{:,.0f}','Monto(Bs.)':'{:,.2f}'}
            st.dataframe(df_f[cols_m].style.format(fmt_f, na_rep='—').map(color_diag, subset=['Diagnóstico']), use_container_width=True, hide_index=True)
            st.markdown("###### 📅 Detalle por Año")
            for anio in sorted(df_f['_anio'].unique()):
                if anio == 0: continue
                df_a = df_f[df_f['_anio'] == anio]
                df_a_se = df_a[~df_a['Estatus_Fiscal'].str.contains('Excluido', na=False)]
                n_tarde = len(df_a_se[df_a_se['Días de Mora/Desviación'] > 0])
                with st.expander(f"{'🟢' if n_tarde==0 else '🔴'} {anio}  ·  {len(df_a)} pagos  ·  {n_tarde} tardíos"):
                    st.dataframe(df_a[cols_m].style.format(fmt_f, na_rep='—').map(color_diag, subset=['Diagnóstico']), use_container_width=True, hide_index=True)

        if tiene_transicion:
            t1, t2, t3 = st.tabs(["📘 Ordinario — Fechas","📙 Especial — Fechas","🔢 Conjunto Total"])
            with t1: render_tabla_fechas(df_fechas[df_fechas['Fase_Fiscal']=='Ordinario'], "Fase Ordinaria — Comparativo de Fechas")
            with t2: render_tabla_fechas(df_fechas[df_fechas['Fase_Fiscal']=='Especial'],  "Fase Especial — Comparativo de Fechas")
            with t3: render_tabla_fechas(df_fechas, "Conjunto Total (Ordinario + Especial)")
        else:
    ...
    for col_ui, mk, md in [(bcv_cols[0],"EUR",eur_d),(bcv_cols[1],"USD",usd_d)]:
        t = md.get("tasa"); f = md.get("fecha","—")
        if t: col_ui.metric(f"Tasa BCV {mk}", f"Bs. {t:,.2f}", delta=f)
    ...
    # ── Resumen de tardíos desglosado por impuesto ───────────
    mask_tarde_rep = df_reporte['Estatus_Fiscal'].str.contains('Pagado Tarde', na=False)
    n_tarde_total  = int(mask_tarde_rep.sum())
    if n_tarde_total > 0:
        st.markdown("#### ⏰ Pagos Tardíos — Desglose por Impuesto")
        tard_desglose = (
            df_reporte[mask_tarde_rep]
            .groupby('Impuesto').size()
            .reset_index(name='Declaraciones Tardías')
            .sort_values('Declaraciones Tardías', ascending=False)
        )
        tard_desglose['Multa 103 (EUR)']  = tard_desglose['Declaraciones Tardías'] * 100
        tard_desglose['Multa 108 (EUR)']  = tard_desglose.apply(
            lambda r: r['Declaraciones Tardías'] * 200
            if any(df_reporte[(df_reporte['Impuesto'] == r['Impuesto']) & mask_tarde_rep]['Fase_Fiscal'].str.lower() == 'especial')
            else 0, axis=1
        )
        # fila total
        tot_row = pd.DataFrame([{
            'Impuesto': '📊 TOTAL',
            'Declaraciones Tardías': tard_desglose['Declaraciones Tardías'].sum(),
            'Multa 103 (EUR)': tard_desglose['Multa 103 (EUR)'].sum(),
            'Multa 108 (EUR)': tard_desglose['Multa 108 (EUR)'].sum(),
        }])
        tard_desglose = pd.concat([tard_desglose, tot_row], ignore_index=True)
        st.dataframe(tard_desglose, hide_index=True, use_container_width=True)

    # ── Resumen de omisiones IVA desglosado ──────────────────
    if 'Multa_Omision_IVA(EUR)' in df_reporte.columns:
        mask_vencido_rep = df_reporte['Estatus_Fiscal'].str.contains('NO PAGADO \\(Vencido\\)', na=False)
        mask_iva_rep     = df_reporte['Impuesto'].str.contains('IVA', na=False)
        df_omit          = df_reporte[mask_vencido_rep & mask_iva_rep].copy()

        if not df_omit.empty:
            st.markdown("#### 🧾 Omisiones IVA — Desglose por Impuesto")
            omit_desglose = (
                df_omit.groupby('Impuesto')
                .agg(
                    Declaraciones=('Estatus_Fiscal', 'count'),
                    Multa_EUR=('Multa_Omision_IVA(EUR)', 'sum')
                )
                .reset_index()
                .rename(columns={'Declaraciones': 'Decl. Omitidas', 'Multa_EUR': 'Multa (EUR)'})
            )
            omit_desglose['Multa (Bs.)'] = (omit_desglose['Multa (EUR)'] * tasa_uso).round(2)
            # fila total
            tot_omit = pd.DataFrame([{
                'Impuesto':      '📊 TOTAL',
                'Decl. Omitidas': omit_desglose['Decl. Omitidas'].sum(),
                'Multa (EUR)':   omit_desglose['Multa (EUR)'].sum(),
                'Multa (Bs.)':   omit_desglose['Multa (Bs.)'].sum(),
            }])
            omit_desglose = pd.concat([omit_desglose, tot_omit], ignore_index=True)
            fmt_omit = {'Multa (EUR)': '{:,.2f}', 'Multa (Bs.)': '{:,.2f}'}
            st.dataframe(omit_desglose.style.format(fmt_omit, na_rep='—'), hide_index=True, use_container_width=True)

    st.markdown("#### 💰 Impacto Financiero Estimado (COT)")

    if 'Ordinario' in fases_presentes and 'Especial' in fases_presentes:
        r1, r2, r3 = st.tabs(["📘 Ordinario","📙 Especial","🔢 Total Conjunto"])
        with r1: st.dataframe(construir_desglose(df_reporte[df_reporte['Fase_Fiscal']=='Ordinario'], tasa_uso, tasa_oficial), hide_index=True, use_container_width=True)
        with r2: st.dataframe(construir_desglose(df_reporte[df_reporte['Fase_Fiscal']=='Especial'],  tasa_uso, tasa_oficial), hide_index=True, use_container_width=True)
        with r3: st.dataframe(construir_desglose(df_reporte, tasa_uso, tasa_oficial), hide_index=True, use_container_width=True)
    else:
        st.dataframe(construir_desglose(df_reporte, tasa_uso, tasa_oficial), hide_index=True, use_container_width=True)

    st.markdown("---")
    excel_data = generar_excel_multitabla(df_reporte)
    st.download_button(
        label="📥 Descargar Reporte Completo (.xlsx)", data=excel_data,
        file_name=f"Contingencia{info['digito']}_{info['fecha_auditoria'].replace('/', '-')}.xlsx",
