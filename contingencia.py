...
.stApp { background: #f4f6fb; color: #1e293b; }
...
h1 { font-size: 1.75rem !important; font-weight: 700 !important; color: #0f172a !important; letter-spacing: -0.02em !important; }
...
[data-testid="stMetricValue"] { color: #0f172a !important; font-size: 1.8rem !important; font-weight: 700 !important; font-family: 'JetBrains Mono', monospace !important; }
...
def obtener_tasa_bcv_euro():
    try:
        ...
        last_update_ve = euro_data.last_update.astimezone(zone)
        fecha_str = last_update_ve.strftime('%d/%m/%Y, %I:%M %p')
        return float(tasa_eur), fecha_str
    except Exception as e:
        st.error(f"Fallo al conectar con BCV: {e}")
...
def procesar_auditoria_completa(df_ops, df_cal, digito_usuario, fecha_contingencia, tipo_cierre_usuario):
    ...
    # Filtro IVA/30: acepta "Total IVA a Pagar" o concepto vacío
    def es_registro_valido_iva(row):
        if row['Renta_Norm'] != 'IVA/30':
            return True
        concepto_limpio = str(row.get(nombre_concepto, '')).lower().strip()
        if concepto_limpio in ['', 'nan', 'none', '---']:
            return True
        if 'total' in concepto_limpio and 'iva' in concepto_limpio:
            return True
        if any(x in concepto_limpio for x in ['pago', 'declaracion', 'cuota']):
            return True
        return False
    ...
    if not df_ops.empty:
        ...
        # Detección de Especial: IVA/35 O ANTICIPO-ISLR (el más temprano gana)
    ...
    # Monto base — redondeado desde origen
    ...
    tasa_calculo = round(float(tasa_eur), 2) if tasa_eur else 0.0

    # Multas — cada operación redondeada a 2 decimales inmediatamente
    ...
    # Redondeo final garantizado sobre todo el DataFrame
    ...
    columnas_finales = [
        ...
        'Periodo Registrado en Excel', 'Fecha Límite SENIAT', 'Fecha de Pago Real',
...
def generar_excel_multitabla(df_detalles):
    ...
    def aplicar_formato_excel(ws):
        ...
        # Formato numérico y ancho de columna
        col_widths = {}
...
archivo_subido = st.file_uploader(
    "📂 Cargar archivo de operaciones (.xlsx)", type=["xlsx"],
...
def aplicar_formato_visual(df):
    """Devuelve un Styler con formato limpio: 2 dec para montos, entero para días."""
    fmt = {}
    for c in COLS_BS:
...
if archivo_subido is not None and not df_calendario.empty:
    ...
    # ══════════════════════════════════════════════════════════
    # AUDITORÍA DE FECHAS — Fecha pagada vs Fecha límite SENIAT
    # ══════════════════════════════════════════════════════════
    ...
    # Filtrar solo filas que tienen fecha de pago real
    ...
    if not df_con_pago.empty:
        ...
        if tiene_transicion:
            ...
            df_ord_f = df_fechas[df_fechas['Fase_Fiscal'] == 'Ordinario']
            df_esp_f = df_fechas[df_fechas['Fase_Fiscal'] == 'Especial']
            with tab_ord_f:
                render_tabla_fechas(df_ord_f, "Fase Ordinaria — Comparativo de Fechas")
            with tab_esp_f:
                render_tabla_fechas(df_esp_f, "Fase Especial — Comparativo de Fechas")
            with tab_total_f:
    ...
    if tasa_oficial:
        st.markdown(
            f'<div class="bcv-badge">💶 Tasa BCV EUR: <strong>Bs.&nbsp;{tasa_oficial:,.2f}</strong>'
            f'&nbsp;&nbsp;·&nbsp;&nbsp;{fecha_oficial}</div>',
            unsafe_allow_html=True
        )
    ...
    if 'Ordinario' in fases_presentes and 'Especial' in fases_presentes:
        tab_res_ord, tab_res_esp, tab_res_tot = st.tabs([
            "📘 Ordinario", "📙 Especial", "🔢 Total Conjunto"
        ])
        with tab_res_ord:
            st.dataframe(
                construir_desglose(df_reporte[df_reporte['Fase_Fiscal'] == 'Ordinario'], tasa_uso),
                hide_index=True, use_container_width=True
            )
        with tab_res_esp:
            st.dataframe(
                construir_desglose(df_reporte[df_reporte['Fase_Fiscal'] == 'Especial'], tasa_uso),
                hide_index=True, use_container_width=True
            )
        with tab_res_tot:
            st.dataframe(
                construir_desglose(df_reporte, tasa_uso),
                hide_index=True, use_container_width=True
            )
    else:
    ...
    st.markdown("""
    <div style="background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;padding:3rem 2rem;
    text-align:center;box-shadow:0 2px 12px rgba(0,0,0,0.06);margin-top:1rem;">
        <div style="font-size:3.5rem;margin-bottom:1rem;">📂</div>
        <div style="font-size:1.1rem;font-weight:600;color:#1e293b;margin-bottom:0.5rem;">
            Carga tu archivo Excel de operaciones para comenzar
        </div>
