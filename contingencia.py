...
def procesar_auditoria_completa(df_ops, df_cal, digito_usuario, fecha_contingencia, tipo_cierre_usuario):
    ...
    # ── CORRECCIÓN 2: Sustitutiva en ISLR-ANUAL se MANTIENE en el cruce
    #    (se marcará como Excluido en determinar_estatus, NO se filtra aquí)
    def es_registro_valido(row):
        ...
        if row['Renta_Norm'] == 'ISLR-ANUAL':
            # NO se filtra la sustitutiva aquí; se gestiona en determinar_estatus
            return True
    ...
    if not df_ops.empty:
        ...
        # ── CORRECCIÓN 1: Detectar quincena real del primer periodo especial ──
        if candidatos_transicion:
            ...
            if trans_yyyymm > min_general_yyyymm:
                ...
                # Detectar quincena real del primer periodo especial
                # Usamos el ÚLTIMO pago registrado en ese periodo (el más reciente)
                df_primer_especial = df_ops[df_ops['YYYYMM'] == trans_yyyymm].sort_values(
                    'Fecha_Operacion_Real', ascending=True, na_position='last'
                )
                quincena_trans = "Primera"  # valor por defecto
                if not df_primer_especial.empty:
                    # Intentar leer columna de quincena original
                    col_q = [c for c in df_ops.columns if 'quincena' in c.lower()]
                    if col_q:
                        # Tomar el último registro (mayor fecha) para capturar la quincena efectiva
                        val_q = str(df_primer_especial.iloc[-1][col_q[0]]).upper()
                        if '2' in val_q or 'SEGUND' in val_q:
                            quincena_trans = "Segunda"
                        elif '1' in val_q or 'PRIMER' in val_q:
                            quincena_trans = "Primera"

                    # Fallback: deducir por día de la fecha de operación real
                    if quincena_trans == "Primera":
                        fechas_validas = df_primer_especial['Fecha_Operacion_Real'].dropna()
                        if not fechas_validas.empty:
                            dia = fechas_validas.iloc[-1].day
                            quincena_trans = "Segunda" if dia > 15 else "Primera"
    ...
    def determinar_estatus(row):
        ...
        if row['_merge'] == 'left_only':
            ...
            # ── CORRECCIÓN 2: Sustitutiva en ISLR-ANUAL → Excluido, sin multa ──
    ...
    # ── CORRECCIÓN 2: Multas fijas excluyen registros marcados como Excluido ──
    df_audit['Multa_Art_103(EUR)'] = np.where(
        df_audit['Estatus_Fiscal'].str.contains('Pagado Tarde') &
        ~df_audit['Estatus_Fiscal'].str.contains('Excluido', na=False),
        100.0, 0.0
    )
    df_audit['Multa_Art_108(EUR)'] = np.where(
        df_audit['Estatus_Fiscal'].str.contains('Pagado Tarde') &
        (df_audit['Fase_Fiscal'].str.lower() == 'especial') &
        ~df_audit['Estatus_Fiscal'].str.contains('Excluido', na=False),
        200.0, 0.0
    )
    ...
    # Registros Excluidos por Sustitutiva: deuda = 0
...
if archivo_subido is not None and not df_calendario.empty:
    ...
    if not df_con_pago.empty:
        ...
        def diagnostico_fecha(row):
            # Excluidos por sustitutiva no tienen diagnóstico de mora
            if 'Excluido' in str(row.get('Estatus_Fiscal', '')):
                return '🟡 Excluido (Sustitutiva)'
        ...
        def render_tabla_fechas(df_f, titulo):
            ...
            # Excluir sustitutivas del conteo de tardíos
            df_sin_excluidos = df_f[~df_f['Estatus_Fiscal'].str.contains('Excluido', na=False)]
            total   = len(df_f)
            tarde   = len(df_sin_excluidos[df_sin_excluidos['Días de Mora/Desviación'] > 0])
            exacto  = len(df_sin_excluidos[df_sin_excluidos['Días de Mora/Desviación'] == 0])
            adelant = len(df_sin_excluidos[df_sin_excluidos['Días de Mora/Desviación'] < 0])
            ...
            m5.metric("🟡 Excluidos", excluidos)
            m6.metric("✅ Cumplimiento", f"{pct_ok}%")
            ...
            for anio in sorted(df_f['_anio'].unique()):
                ...
                df_a_sin_excl = df_a[~df_a['Estatus_Fiscal'].str.contains('Excluido', na=False)]
                n_tarde = len(df_a_sin_excl[df_a_sin_excl['Días de Mora/Desviación'] > 0])
